...
export const insertUser = async (UserDTO) => {
    const sql = 'INSERT INTO Users (user_id, user_pw, u_name, u_phone, u_email, u_gender, u_birth, c_content) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
...
export const findUserById = async (user_id) => {
    const sql = 'SELECT * FROM Users WHERE user_id = ?';         // ?에는 나중에 값이 들어감
...
export const updateUserPassword = async (user_id, new_pw) => {
    const sql = 'UPDATE Users SET user_pw = ? WHERE user_id = ?';
